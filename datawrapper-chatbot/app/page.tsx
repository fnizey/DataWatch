...
export default function Home() {
  ...
  async function handleSend(userPrompt: string) {
    ...
    try {
      ...
      if (!planRes.ok) {
        throw new Error(planData.error ?? 'Chart-planlegging feilet');
      }
      ...
      addMessage(
        'assistant',
        `Lager ${plan.title} (${plan.datawrapperType}). ${plan.reasoning}`
      );

      // ── Step 2–4: Create, upload and publish via Datawrapper ──────────
      updateStep(2);
      ...
      // Show "uploading data" step while waiting
      updateStep(3);

      const createData = await createRes.json();

      if (!createRes.ok) {
        throw new Error(createData.error ?? 'Oppretting av chart feilet');
      }

      updateStep(4);
      // Small pause so "Publiserer…" is visible
      await new Promise((r) => setTimeout(r, 800));

      updateStep(5); // All done
      ...
      addMessage(
        'assistant',
        `✅ Ferdig! Chartet er publisert i Datawrapper. Chart ID: ${createData.chartId}`
      );
    } catch (err: unknown) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-newsroom-bg">
      {/* ── Top bar ──────────────────────────────────────────────────── */}
      <header className="bg-newsroom-dark border-b border-gray-800 px-6 py-0 h-14 flex items-center gap-4 shrink-0">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-5">
              ...
              <ol className="space-y-2.5">
                {[
                  ...
                  'Kopier embed-kode til artikkelen',
                ].map((step, i) => (
      ...
      <footer className="h-10 border-t border-gray-200 bg-white flex items-center justify-center">
        <p className="text-xs text-gray-400">
          AI Datawrapper Beta · Powered by OpenAI + Datawrapper API
        </p>
      </footer>
    </div>
  );
