...
export default function FileUpload({ onUpload, isLoading }: FileUploadProps) {
  ...
  async function processFile(file: File, sheet?: string) {
    ...
    try {
      ...
      if (data.sheetNames && data.sheetNames.length > 1 && !sheet) {
  ...
  async function handleSheetChange(sheet: string) {
    ...
    if (fileRef.current) await processFile(fileRef.current, sheet);
  }

  function handleFileChange(e: ChangeEvent<HTMLInputElement>) {
    const file = e.target.files?.[0];
    if (file) { setSheetNames([]); setSelectedSheet(''); processFile(file); }
  }
    ...
    <div className="space-y-3">
      <div
        ...
        className={`relative border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all duration-200 select-none
          ${isDragging ? 'border-accent bg-red-50 scale-[1.01]' : fileName ? 'border-green-400 bg-green-50' : 'border-gray-200 bg-gray-50 hover:border-accent hover:bg-red-50'}
          ${busy ? 'opacity-60 cursor-not-allowed' : ''}`}
      >
        <input ref={inputRef} type="file" accept=".csv,.xls,.xlsx" className="hidden" onChange={handleFileChange} disabled={busy} />
        {busy ? (
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-gray-700">Dra hit eller <span className="text-accent">velg fil</span></p>
      ...
      {sheetNames.length > 1 && (
        <div>
          <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-1.5">Velg ark</label>
          <select
            value={selectedSheet}
            onChange={e => handleSheetChange(e.target.value)}
            disabled={busy}
            className="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 outline-none focus:border-accent bg-white"
          >
