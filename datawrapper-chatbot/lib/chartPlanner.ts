...
export interface ChartPlan {
  ...
  valueFormat: string | null; // D3 format string, e.g. "0.00" for 2 decimals
}
...
- d3-bars: Stolpediagram (horisontal). Bruk for kategorier, rangeringer, sammenligninger.
- d3-bars-stacked: Stablet stolpediagram. Bruk for deler av en helhet over kategorier.
...
- d3-maps-choropleth: Koroplettkart. Bruk for geografiske data med verdier.

Regler for y-akse og formatering:
- Hvis brukeren nevner "y-akse fra X til Y", "start på X", "sett akse til X-Y" e.l. → sett yAxisMin og yAxisMax
- Hvis brukeren nevner "desimal", "desimaler", "decimal" → sett valueFormat:
  - "ingen desimaler" / "0 desimaler" → "0"
  - "én desimal" / "1 desimal" → "0.0"
  - "to desimaler" / "2 desimaler" → "0.00"
  - "tre desimaler" / "3 desimaler" → "0.000"
- Hvis ikke nevnt: yAxisMin og yAxisMax = null, valueFormat = null

Returnerer KUN gyldig JSON uten markdown.`;
...
export async function planChart(
  ...
  const columnSummary = columns
    .map(
      (c) =>
        `  - "${c.name}" [${c.type}]: ${c.sample.filter(Boolean).slice(0, 4).join(', ')}`
    )
    .join('\n');
