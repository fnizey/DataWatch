...
function getToken(): string {
  ...
  if (!token) throw new Error('DATAWRAPPER_API_KEY er ikke satt');
  return token;
}
...
async function handleResponse(res: Response, label: string): Promise<unknown> {
  if (!res.ok) {
    let err = ''; try { err = await res.text(); } catch { err = res.statusText; }
    throw new Error(`${label} feilet (HTTP ${res.status}): ${err}`);
  }
...
export interface CreatedChart { id: string; title: string; type: string; }
export interface PublishResult { chartId: string; publicUrl: string; embedCode: string; }

export async function createChart(type: string, title: string): Promise<CreatedChart> {
  const res = await fetch(`${DW_BASE}/charts`, { method: 'POST', headers: jsonHeaders(), body: JSON.stringify({ type, title }) });
...
export async function updateChartMetadata(chartId: string, opts: UpdateChartOpts): Promise<void> {
  ...
  if (opts.baseColor) visualize['base-color'] = opts.baseColor;
  if (opts.showLabels !== undefined) visualize['labels'] = opts.showLabels;
  if (opts.referenceLineValue !== null && opts.referenceLineValue !== undefined) {
    visualize['rules'] = [{
      value: opts.referenceLineValue,
      label: opts.referenceLineLabel || '',
      color: '#666666',
      opacity: 100,
      strokeType: 'dashed',
      width: 2,
  ...
  if (opts.valueFormat && opts.yColumn) {
    columnFormat[opts.yColumn] = { type: 'auto', 'number-format': opts.valueFormat, 'number-divisor': 0, 'number-append': '', 'number-prepend': '' };
  }

  const body: Record<string, unknown> = {
    title: opts.title,
    metadata: {
      describe: { intro: opts.description || '', 'source-name': opts.source || '', 'source-url': '', byline: opts.byline || '' },
      annotate: { notes: opts.notes || '' },
      ...(Object.keys(visualize).length > 0 ? { visualize } : {}),
      ...(Object.keys(columnFormat).length > 0 ? { data: { 'column-format': columnFormat } } : {}),
    },
  };

  const res = await fetch(`${DW_BASE}/charts/${chartId}`, { method: 'PATCH', headers: jsonHeaders(), body: JSON.stringify(body) });
  await handleResponse(res, 'Oppdater metadata');
}

export async function publishChart(chartId: string): Promise<PublishResult> {
  const res = await fetch(`${DW_BASE}/charts/${chartId}/publish`, { method: 'POST', headers: jsonHeaders() });
  ...
  return { chartId, publicUrl, embedCode: buildEmbedCode(chartId, cdnUrl) };
}
